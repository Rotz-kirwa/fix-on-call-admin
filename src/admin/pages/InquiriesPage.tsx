  ...
  const fetchInquiries = async () => {
    try {
      ...
      const rows = ((res.data?.conversations || []) as SupportConversationDTO[])
        ...
        .sort((a, b) => {
          const aTime = parseApiDate(a.last_message_at || a.updated_at || a.created_at || "")?.getTime() || 0;
          const bTime = parseApiDate(b.last_message_at || b.updated_at || b.created_at || "")?.getTime() || 0;
          return bTime - aTime;
        });
      ...
      <aside className="xl:col-span-4 rounded-2xl border border-slate-200 bg-white overflow-hidden">
        ...
        <div className="p-3 space-y-2 max-h-[70vh] overflow-auto">
          {conversations.map((inquiry) => (
            ...
            >
              {!isRead(inquiry.id) && (
                <div className="mb-2 rounded-md border border-blue-300 bg-blue-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-blue-700">
                  New
                </div>
              )}
              <div className="flex items-center justify-between gap-2">
              ...
              <p className="text-[10px] text-slate-500 mt-1">
                Sent: {formatNairobiDateTime(sentAt(inquiry))}
              </p>
              <div
                className={`mt-2 flex items-center justify-between rounded-lg px-2 py-1 text-[10px] ${
                  isRead(inquiry.id) ? "bg-emerald-50 border border-emerald-200" : "bg-blue-100 border border-blue-200"
                }`}
              >
                {isRead(inquiry.id) ? (
                  <>
                  ...
                  <>
                    <span className="text-blue-700 font-semibold">New</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-900 mt-1">
                    {formatNairobiDateTime(sentAt(selected))}
                  </p>
                </div>
              </div>
